...
namespace FP2_Knux_Objects.Common
{
    public class ItemMonitor : FPBaseObject
    {
        ...
        private FPBaseObject targetObject;
        ...
        // Values from the asset bundle for the stage mod that uses this class.
        // The default sound names match things from Sonic Origins and Sonic Mania, but can be changed by the mod overwriting these values.
        // The Invincibility Jingle will fall back on playing the game's own unused one if left null.
        public AssetBundle assetBundle;
        public string ringSound = "SFX_B5_Ring";
        public string hyperRingSound = "HyperRing";
        public string woodShieldSound = "SFX_AF_BlueShield";
        public string earthShieldSound = "S014_S41_LightningShield";
        public string waterShieldSound = "S012_S3F_BubbleShield";
        public string fireShieldSound = "S011_S3E_FireShield";
        public string metalShieldSound = "SFX_B6_SpikesMove";
        public string hitSound = "SFX_C1_Destroy";
        public AudioClip InvincibilityJingle = null;

        private new void Start()
        {
            // Check if an asset bundle has been set. If not, then report it and kill this monitor.
            if (assetBundle == null)
            {
                Plugin.consoleLog.LogError($"Monitor without an asset bundle found at {transform.position}! Removing.");
        ...
        private void State_GetItem()
        {
            ...
            switch (itemType)
            {
                ...
                case MonitorTypes.Crystals:
                    FPAudio.PlaySfx(assetBundle.LoadAsset<AudioClip>(ringSound));
                    for (int i = 0; i < 10; i++)
                        GiveCrystal();
                    break;
                ...
                case MonitorTypes.LifePetals:
                    FPAudio.PlaySfx(assetBundle.LoadAsset<AudioClip>(hyperRingSound));
                ...
                case MonitorTypes.Invincibility:
                    ...
                    // Stop any playing jingles.
                    FPAudio.StopJingle();

                    // Play the invincibility theme (either the base game's unused one or a custom one if provided).
                    if (InvincibilityJingle == null) FPAudio.PlayJingle(0);
                    else FPAudio.PlayJingle(InvincibilityJingle);
                    break;
            ...
            void GiveShield(string type)
            {
                ...
                {
                    case "Wood":
                        FPAudio.PlaySfx(assetBundle.LoadAsset<AudioClip>(woodShieldSound));
                        FPPlayerPatcher.player.shieldID = 0;
                        break;
                    case "Earth":
                        FPAudio.PlaySfx(assetBundle.LoadAsset<AudioClip>(earthShieldSound));
                        FPPlayerPatcher.player.shieldID = 1;
                        break;
                    case "Water":
                        FPAudio.PlaySfx(assetBundle.LoadAsset<AudioClip>(waterShieldSound));
                        FPPlayerPatcher.player.shieldID = 2;
                        break;
                    case "Fire":
                        FPAudio.PlaySfx(assetBundle.LoadAsset<AudioClip>(fireShieldSound));
                        FPPlayerPatcher.player.shieldID = 3;
                        break;
                    case "Metal":
                        FPAudio.PlaySfx(assetBundle.LoadAsset<AudioClip>(metalShieldSound));
        ...
        public void BoxHit()
        {
            ...
            FPAudio.PlaySfx(assetBundle.LoadAsset<AudioClip>(hitSound));
