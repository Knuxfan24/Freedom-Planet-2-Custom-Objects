...
namespace FP2_Knux_Objects.HiddenPalaceZone
{
    public class HPZPipeValve : FPBaseObject
    {
        // FPBaseObject stuff.
        ...
        // The default sound name is taken from my Hidden Palace Zone mod, but can be changed by the mod overwriting it.
        public AssetBundle assetBundle;
        public string geyserSound = "classic_watergeyser";

        private new void Start()
        {
            // Check if an asset bundle has been set. If not, then report it and kill this pipe valve.
            if (assetBundle == null)
            {
                Plugin.consoleLog.LogError($"Pipe Valve without an asset bundle found at {transform.position}! Removing.");
        ...
        private void State_Default()
        {
            ...
            while (FPStage.ForEach(FPPlayer.classID, ref objRef))
            {
                ...
                if (FPCollision.CheckOOBB(this, hbTouch, objRef, fPPlayer.hbTouch))
                {
                    ...
                    FPAudio.PlaySfx(assetBundle.LoadAsset<AudioClip>(geyserSound));
